...
the port exposed on localhost.  You can also use ```docker run -it --link some-mongo:mongo --rm mongo sh -c 'exec mongo "$MONGO_PORT_27017_TCP_ADDR:$MONGO_PORT_27017_TCP_PORT/test"'``` to launch the client inside the container.  I'll eventually add a Dockerfile for the app to
